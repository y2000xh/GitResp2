...
@WebServlet("/findMeetingDay7Servlet")
public class findMeetingDay7Servlet extends HttpServlet {
    //已取消
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //调用数据库 查询
        //获取Service返回的数据
        List<Meeting> ml = MeetingService.findMeetingByStatus1(username);
        ...
        System.out.println("findMeetingDay7Servlet.get=========" + as);
        Gson gson = new Gson();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //调用数据库 查询
        //获取Service返回的数据
        List<Meeting> ml = MeetingService.findMeetingDay7(username);
        //将java类型的al数据通过Gson转换为Json格式的数据
        //实例化Gson对象
        ArrayList<ArrayList> as = new ArrayList<>();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        ...
        System.out.println("findMeetingDay7Servlet.post----------" + as);
        Gson gson = new Gson();
        //通过PrintWiter 将数据返回给页面
        PrintWriter pw = response.getWriter();
        pw.print(gson.toJson(as));
