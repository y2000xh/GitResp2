...
@WebServlet("/findMeetingByMnameServlet")
public class findMeetingByMnameServlet extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //调用数据库 查询
        //获取Service返回的数据
        List<Meeting> ml = MeetingService.findMeetingByMname(mid);
        ...
        for (int i = 0; i < ml.size(); i++) {
            ...
            l.add(ml.get(i).getMcount().toString());
            ...
            l.add(ml.get(i).getEmp().getEid().toString());
            l.add(ml.get(i).getEmp().getEname().toString());
            l.add(ml.get(i).getStatus().toString());
            l.add(ml.get(i).getMnotes().toString());
            as.add(l);
        }
